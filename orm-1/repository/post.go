...
import (
	"context"
	"database/sql"
	"elegantGo/orm-1/repository/entity"
	"fmt"
)
...
func (repo *Post) Fetch(ctx context.Context, id int) *entity.Post {
	...
	query := fmt.Sprintf("SELECT posts.`id`, posts.`hash_id`, posts.`user_id`, posts.`title`, posts.`content`, posts.`view`, posts.`create_time`, posts.`update_time`, users.`nickname`, users.`avatar` FROM posts, users WHERE posts.user_id=users.id AND posts.`id`= %d", id)
...
func (repo *Post) FetchMany(ctx context.Context) []*entity.Post {
	...
	query := "SELECT posts.`id`, posts.`hash_id`, posts.`user_id`, posts.`title`, posts.`content`, posts.`view`, posts.`create_time`, posts.`update_time`, users.`nickname`, users.`avatar` FROM posts, users WHERE posts.user_id=users.id ORDER BY posts.create_time DESC"
	rows, _ := repo.db.QueryContext(ctx, query)
	defer rows.Close()
