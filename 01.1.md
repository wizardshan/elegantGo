...
这两行之前增加类似的代码逻辑。
```
这是在实际开发中很典型的业务场景，从数据库获取数据，然后把数组转换为合适的业务数据，最后对外输出。
这段代码存在什么问题？读者可以停下来思考几分钟。

通过阅读这一段代码的过程中可以发现，我们在不断的推测新变量和回忆已存在变量的作用，当新变量的加入和老变量的交替出现，
多个变量在我们的思维里不停的跳跃，再加上数据的各种格式转换，同时还要试图理解代码的意图，最终脑子变成一团浆糊。
...
也有这样的台阶可以缓解焦虑，增加人的安全感，原理是每隔一段距离增加一个平台，把长长的台阶分成一节一节的小段，上下台阶的时候每隔一段时间可以在平台上停留休息，放松一下紧张的心情，平复心态。
...
1、业务扩展功能<br>
日常开发中大部分时候都是在迭代开发，在老业务上面进行修改或扩展功能，在做需求变更的时候，我们需要先去理解相关的代码以及上下文涉及的代码，然后再动手开发，
